...
uploaded_file = st.file_uploader(" ", type=['xlsx'],accept_multiple_files=True) #Only accepts excel file format


if uploaded_file is not None:     
    new_data = pd.read_excel(uploaded_file)
    new_data['Month'] = new_data['Month'].apply(lambda x: x.strftime('%B-%Y'))
    
    
    
    hwe_model_mul_add = ExponentialSmoothing(new_data["Sales"], seasonal = "mul", trend = "add", seasonal_periods = 12).fit()
